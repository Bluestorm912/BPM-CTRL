...
const Shop = () => {
  ...
  const handleAuth = async (event: React.FormEvent) => {
    ...
    try {
      if (authMode === "login") {
        ...
        toast({ title: "Welcome back", description: "Your shop account is active." });
      } else {
      ...
      toast({ title: "Account action failed", description: err.message, variant: "destructive" });
    }
  };
  ...
  const handleSaveAddress = async (event: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: "Address saved", description: "This address is ready for future checkouts." });
      setAddressForm((current) => ({ ...current, addressLine1: "", addressLine2: "", city: "", state: "" }));
    } catch (err: any) {
      toast({ title: "Could not save address", description: err.message, variant: "destructive" });
    }
  };

  const requireLogin = (productName: string) => {
    toast({
      title: "Shop account required",
      description: `Sign in or register to interact with ${productName}.`,
      variant: "destructive",
    });
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <div className="fixed top-0 left-0 right-0 z-50 border-b border-border bg-background/90 backdrop-blur-xl">
        <div className="h-10 overflow-hidden border-b border-border/60 bg-primary text-primary-foreground">
          <div className="flex h-full items-center whitespace-nowrap animate-[wave_18s_linear_infinite]">
            {Array.from({ length: 8 }).map((_, index) => (
              <span key={index} className="font-display text-xs tracking-[0.18em] uppercase px-5">
                BPM CTRL SHOP - Accounts unlock order history, saved addresses, loyalty points
              </span>
            ))}
          </div>
        </div>
        <div className="h-14 px-4 md:px-6 flex items-center gap-4">
          ...
          <nav className="ml-auto hidden md:flex items-center gap-6 font-display text-xs tracking-[0.18em] uppercase text-muted-foreground">
            {CATEGORIES.map((item) => (
      ...
      <main className="pt-24">
        <section className="min-h-[58vh] flex items-end relative overflow-hidden">
          ...
          <div className="relative z-10 px-4 md:px-6 pb-10 md:pb-14 max-w-5xl">
            <p className="font-display text-xs tracking-[0.32em] text-primary uppercase mb-3">Shop BPM CTRL</p>
            ...
            <p className="text-lg text-orange-amber/75 font-body max-w-2xl">
              Register once to save addresses, collect loyalty points, and keep your BPM CTRL order history close.
            </p>
          </div>
        </section>
        ...
        <section className="px-4 md:px-6 py-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-border border border-border">
            {products.map((product) => (
              <article key={product.id} className="group bg-card min-h-[520px] flex flex-col">
                ...
                <div className="p-5 flex-1 flex flex-col">
                  ...
                  <p className="text-sm text-muted-foreground font-body mb-5">
                    Designed for broadcasts, late nights, and the people who keep the signal moving.
                  </p>
                  <div className="mt-auto flex items-center justify-between gap-3">
                    ...
                    <Button
                      variant="neon"
                      size="sm"
                      onClick={() => (product.gated && !user ? requireLogin(product.name) : toast({ title: "Product selected", description: "Cart checkout is the next build step." }))}
                    >
                      {product.gated && !user ? "Login" : "View"}
                    </Button>
                  </div>
                </div>
              </article>
        ...
        <section id="shop-account" className="px-4 md:px-6 py-10 md:py-14 border-t border-border">
          <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-6">
            <aside className="glow-border-orange rounded-2xl bg-card p-5 md:p-6 h-fit">
              <div className="flex items-center gap-3 mb-5">
                ...
                <div>
                  <p className="font-display text-xs tracking-[0.24em] text-primary uppercase">Shop Account</p>
                  <h2 className="font-display text-xl font-black text-foreground">{user ? "Member Dashboard" : "Create Access"}</h2>
                </div>
              </div>
              ...
              {!user ? (
                <form onSubmit={handleAuth} className="space-y-4">
                  ...
                  {authMode === "register" && (
                    <>
                      <div>
                        <Label className="font-display text-xs tracking-wider text-muted-foreground uppercase">Full Name</Label>
                      ...
                      <div>
                        <Label className="font-display text-xs tracking-wider text-muted-foreground uppercase">Phone</Label>
                        <Input value={authForm.phone} onChange={(e) => setAuthForm({ ...authForm, phone: e.target.value })} className="mt-1 bg-muted border-border" />
                      </div>
                    </>
                  )}
                  <div>
                    <Label className="font-display text-xs tracking-wider text-muted-foreground uppercase">Email</Label>
                  ...
                  <div>
                    <Label className="font-display text-xs tracking-wider text-muted-foreground uppercase">Password</Label>
                  ...
                  <Button variant="neon" type="submit" className="w-full">
                    {authMode === "register" ? "Create Shop Account" : "Sign In"}
                  </Button>
                </form>
              ) : (
                <div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
              <div className="glow-border-orange rounded-2xl bg-card p-5 md:p-6">
                <div className="flex items-center gap-3 mb-5">
                  <MapPin className="w-5 h-5 text-primary" />
                  <h3 className="font-display text-xl font-black text-foreground">Saved Addresses</h3>
                </div>
                ...
                {user ? (
                  <>
                    ...
                    <div className="space-y-3">
                      ...
                      {addresses.length === 0 && <p className="text-sm text-muted-foreground font-body">No saved addresses yet.</p>}
                    </div>
                  </>
                ) : (
                  <p className="text-sm text-muted-foreground font-body">Create or sign into a shop account to save delivery addresses.</p>
                )}
              </div>
              ...
              <div className="glow-border-orange rounded-2xl bg-card p-5 md:p-6">
                <div className="flex items-center gap-3 mb-5">
                  ...
                  <h3 className="font-display text-xl font-black text-foreground">Order History</h3>
                </div>
                {user ? (
                  <div className="space-y-3">
                    {orders.map((order) => (
                    ...
                    {orders.length === 0 && <p className="text-sm text-muted-foreground font-body">Your future orders will appear here.</p>}
                  </div>
                ) : (
                  <p className="text-sm text-muted-foreground font-body">Sign in to see order history and loyalty rewards.</p>
                )}
              </div>
            </div>
          </div>
